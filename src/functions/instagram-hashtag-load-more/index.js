...
export async function handler(event) {
  ...
  const tag_name = request.uri.split('/')[4];
  ...
  try {
    ...
    const response = await fetchHashtagTimeline({
      tag_name,
      first: searchParams.get('first') || 12,
      after: searchParams.get('after')
    });
    const { edges, page_info } = response.data.hashtag.edge_hashtag_to_media;
    const body = JSON.stringify({
      edges,
      'load-more-src': `https://www.stalkool.com/api/instagram/hashtag/${tag_name}?after=${
